        ...
        if (packet instanceof S29PacketSoundEffect) {
            ...
            switch (soundEffect.getSoundName()) {
                ...
                case "mob.villager.haggle":
                    if (QualityOfLife.qolDisableJerryChineGunSounds && InventoryUtils.getSlot("Jerry-chine Gun") == InventoryUtils.getCurrentSlot()) {
                        if (event.isCancelable()) event.setCanceled(true);
                    }
                    break;
                case "mob.endermen.portal":
                    if (QualityOfLife.qolDisableAoteSounds && InventoryUtils.getSlot("Aspect of the End") == InventoryUtils.getCurrentSlot()) {
                        if (event.isCancelable()) event.setCanceled(true);
                    }
                    break;
                case "mob.zombie.death":
                    if (QualityOfLife.qolDisableZombieRareDrops) {
