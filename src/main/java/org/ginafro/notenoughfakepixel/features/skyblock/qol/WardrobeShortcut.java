...
public class WardrobeShortcut {
    ...
    public void onKeyPress(InputEvent.KeyInputEvent event) {
        if (!QualityOfLife.qolShortcutWardrobe) return;
        if (!ScoreboardUtils.currentGamemode.isSkyblock()) return;
        EntityPlayerSP player = Minecraft.getMinecraft().thePlayer;

        // Check if the single key is pressed
        boolean keyPressed = Keyboard.isKeyDown(QualityOfLife.qolWardrobeKey);

        // If the key is pressed and it's not already active
        if (keyPressed && !activeKeySet.contains(QualityOfLife.qolWardrobeKey)) {
            // Execute the action
            player.sendChatMessage("/wardrobe");

            // Mark this key as active
            activeKeySet.add(QualityOfLife.qolWardrobeKey);
        }

        // Clear key from activeKeySet when released
        if (!Keyboard.isKeyDown(QualityOfLife.qolWardrobeKey)) {
            activeKeySet.remove(QualityOfLife.qolWardrobeKey);
        }
    }
